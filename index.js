...
function flattenBody(body) {
  ...
  body.forEach(statement => {
    output = [...output, flattenStatement(statement)]
  })
...
function flattenStatement(statement) {
  if (statement.type == "ReturnStatement"){
    ...
    return flattenExpr("~out", statement.argument)
  }
  if (statement.type == "ExpressionStatement"){
    return flattenExpr(statement.expression.left.name, statement.expression.right)
  }

}

function Operation(op, target, left, right ) {
  return { op, target, left, right }
}

function flattenExpr(left, right) {
  ...
  if (right.type == "Identifier") {
    ...
    if (right.right.type == "Identifier" || right.right.type == "Literal") {
      var2 = right.right.type == "Identifier" ? right.right.name : Number(right.right.raw)
      sub2 = []
    } else {
      var2 = mksymbol()
...
function flatcodeToR1cs(inputs, flatcode) {
  ...
  flatcode.forEach(({op, target, left, right}) => {
...
function assignVariables(inputs, inputVars, flatcode) {
  ...
  flatcode.forEach(({op, target, left, right}) => {
    if (op == 'set') {
      ...
      assignment[varz.indexOf(target)] = grabVar(varz, assignment, left) + grabVar(varz, assignment, right)
    } else if (op === "-") {
      assignment[varz.indexOf(target)] = grabVar(varz, assignment, left) - grabVar(varz, assignment, right)
    } else if (op === "*") {
      assignment[varz.indexOf(target)] = grabVar(varz, assignment, left) * grabVar(varz, assignment, right)
